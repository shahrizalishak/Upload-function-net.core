...
namespace eForm.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : eFormServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("eForm", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
